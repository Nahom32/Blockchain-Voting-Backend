...
export default function makeOrganizationList(){
    return({
        ...
        getOraganizations,
        updateOraganization,
        getOraganizationsByUserId,
        getOrganizationsByUserEmail,
        getOrganizationsUserMemberOf
    });
    ...
    async function getOraganizationsByUserId(id: string):Promise<Organization[]>{
        const oraganizations = await prisma.organizations.findMany({
            where:{
                userId: id
            },
    ...
    async function getOrganizationsUserMemberOf(userId: string): Promise<Organization[]> {
        const organizations = await prisma.organizations.findMany({
            where: {
                members: {
                    some: {
